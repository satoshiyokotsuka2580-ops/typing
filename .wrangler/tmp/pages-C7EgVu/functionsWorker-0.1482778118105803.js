...
var MODES = /* @__PURE__ */ new Set(["english", "japanese"]);
var LEVELS = /* @__PURE__ */ new Set(["beginner", "intermediate", "advanced"]);
async function onRequestGet({ request, env: env2 }) {
  if (!env2.DB) return json({ error: "D1 binding DB is not configured" }, 503);
...
async function onRequestPost({ request, env: env2 }) {
  ...
  if (![kps, words, accuracy].every(Number.isFinite) || kps < 0 || kps > 30 || words < 0 || words > 1e3 || accuracy < 0 || accuracy > 100) return json({ error: "Score out of range" }, 400);
  await env2.DB.prepare(`INSERT INTO rankings (username, mode, level, duration, kps, words, accuracy) VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(username, mode, level, duration, Math.round(kps * 1e3) / 1e3, Math.floor(words), Math.round(accuracy)).run();
  return json({ ok: true }, 201);
}
__name(onRequestPost, "onRequestPost");

// ../.wrangler/tmp/pages-C7EgVu/functionsRoutes-0.5926120671614705.mjs
var routes = [
  {
